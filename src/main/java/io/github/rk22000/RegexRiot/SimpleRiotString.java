...
class BasicRiotString implements SimpleRiotString {
    ...
    BasicRiotString(String seed) throws IllegalArgumentException {
        if (seed.isEmpty())
            throw new IllegalArgumentException(EMPTY_SEED_EXCEPTION);
        string = seed;
        unitChain = false;
    }
...
// extender = null;
// }
// ChildRiotString(SimpleRiotString mother, SimpleRiotString father,
// Function<SimpleRiotString, SimpleRiotString> extender) {
// super(extender.apply(father).toString(),
// extender.apply(father).isUnitChain());
// this.mother = mother;
// this.father = father;
